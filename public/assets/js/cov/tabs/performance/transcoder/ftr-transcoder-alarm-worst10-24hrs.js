...
covFeatureLoaders['tab-performance']['tab-performance-sec-transcoder'].push(
    ...
    )
    {
        ...
        // --
        var
            featureId = sectionId + '-alarm-worst10-24hrs'
            ;
        ...
        .append(
            ...
            .append(
                ...
                .addClass('ftr-header')
                .text('Worst 10 Performing Transcoder Nodes with High Severity (Sev 1, 2) Alarms (24 hours)')
            ...
            .append(
                ...
                .append(
                    ...
                    .append($('<li>').text('Worst 10 performing Transcoder Nodes are sorted based on the total count of Severity (Sev 1,2) Alarms '))
        ...
        var cellNames = [
            {id: featureId + '-table-Name',        text: 'Xcoder Name'},
            {id: featureId + '-table-IP',          text: 'Xcoder IP'},
            {id: featureId + '-table-region',      text: 'Region'},
            {id: featureId + '-table-Sev1-2',      text: 'Sev1, 2 Alarms',    class: 'text-right'},
        ...
        loadDataApiQuery(
            '/api/clinear/transcoder/alarms/hot/10/' + dateProvided,
            function (response)
            {
                var dataArray = response.data.responseTranscoderHotAlarms10;
                ...
                else
                {
                    ...
                    for (index in dataArray[dateProvided])
                    {
                        ...
                        // Add a cell with the transcoder name
                        newRow.append(
                            $('<td>')
                            .html(stream['transcoderName'])
                        );

                        // Add a cell with the transocder IP
                        newRow.append(
                            $('<td>')
                            .html(stream['transcoderIP'])
                        );

                        // Add a cell with the region
                        newRow.append(
                            $('<td>')
                            .html(stream['region'])
                        );
                        ...
                        // Add a cell with the sev1 & 2 count
                        newRow.append(
                            $('<td>')
                            .addClass('text-right')
                            .html(stream['alarmNum_sev_012'])
                        );
