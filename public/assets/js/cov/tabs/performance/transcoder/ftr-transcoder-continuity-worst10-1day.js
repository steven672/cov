...
covFeatureLoaders['tab-performance']['tab-performance-sec-transcoder'].push(
    function performance_transcoder_loadData_continuity_worst10_1day
    (
        dateProvided = null,
        sectionId = null
    )
    {
        ...
        // --
        var
            featureId = sectionId + '-continuity-worst10-1day'
        ...
        .append(
            ...
            .append(
                ...
                .addClass('ftr-header')
                .text('Transcoder continuity Worst 10 Channel (1 day)')
            ...
            .append(
                ...
                .append(
                   ...
                   .append($('<li>').text('Worst 10 channles are sorted based on most failed event counts captured in API'))
               )
       ...
       ;
        ...
        var cellNames = [
            {id: featureId + '-table-channelName',       text: 'Channel Name'},
            {id: featureId + '-table-transcoderNode',    text: 'Transcoder Node'},
            {id: featureId + '-table-failed',            text: 'Fail',                class: 'text-right'},
            {id: featureId + '-table-successRate',       text: 'Success Rate (%)',    class: 'text-right'},

        ];

        // Run the query specified
        loadDataApiQuery(
            '/api/clinear/transcoder/continuity/hot/10/' + dateProvided,    // The suffix of the URL for the API query, such as '/api/cdvr/legacy/cs/sites/2017-01-14' -- NOT 'http://dev-ccm-php//api/cdvr/legacy/cs/sites/2017-01-14'
            function (response)
            {
                var dataArray = response.data.responseTranscoderHotContinuity10;
                ...
                else
                {
                    ...
                    for (index in dataArray[dateProvided])
                    {
                        ...
                        newRow.append(
                            $('<td>')
                            .html(stream['channelName'])
                        );

                        // Add a cell with the stream name/ID
                        newRow.append(
                            $('<td>')
                            .html(stream['transcoderNode'])
                        );
