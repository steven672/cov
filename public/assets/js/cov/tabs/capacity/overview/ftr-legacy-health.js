covFeatureLoaders['tab-capacity']['tab-capacity-sec-overview'].push(
    function capacity_cdvr_legacyHealth
    (
        dateProvided = null,
        sectionId = null
    )
    {
        ...
        // --
        var
            featureId = sectionId + '-legacy-health',
        ...
        capacity_cdvr_buildTableFromAPIQuery(
            ...
            apiQuery = '/api/cdvr/legacy/health/' + dateProvided,
            codeToRunOnSuccess = function (response)
            {
                ...
                for (rowIndex in $('#' + tableId + ' tbody tr'))
                {
                    ...
                    if (+(rowIndex) >= 0)
                    {
                        buildTargetObject(
                            objectType = 'span',
                            newObjId = sectionId + '-legacy-overall-status' + rowIndex,
                            parentId = sectionId + '-legacy-region' + rowIndex,
                            prepend = true
                        )
                        .css('display', 'none')
                        .text(0)
                ...
                for (index in dataArray)
                {
                    ...
                    var statusId = sectionId + '-legacy-overall-status' + index;
                    var cellId = sectionId + '-legacy-region' + index;
                ...
                makeTableSortable(
                    tableId = sectionId + '-legacy-table',
                    columnHeaderId = sectionId + '-legacy-region'
                );
